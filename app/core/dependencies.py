...
async def admin_required(current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Just only exit admin")
    return current_user
...
async def dean_required(current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "dean":
        raise HTTPException(status_code=403, detail="Just only exit dean")
    return current_user
...
async def teacher_required(current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "teacher":
        raise HTTPException(status_code=403, detail="Just only exit teacher")
    return current_user
