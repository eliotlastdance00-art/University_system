...
from app.auth.schemas import SaveRefreshToken, VerifyOtpRequest, SendOtpRequest
...
class AuthService:
    ...
    async def send_otp(self, request: SendOtpRequest, response: Response):
        try:
            ...
            logging.error(f"Send OTP Error: {str(e)}", exc_info=True)
        
            # WAGTLAÝYNÇA: Ekrana hakyky ýalňyşlygy çykarýarys (Mysal üçin: FileNotFoundError ýa-da SMTP Authentication Error)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
                detail=f"Failed to send OTP email. Internal Error: {str(e)}"
            ) from e

    async def verify_otp(self, request: VerifyOtpRequest, otp_token: str, response: Response):
        try:
            if not otp_token:
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED, 
                    detail="Missing OTP token"
                )
