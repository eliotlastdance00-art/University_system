...
class AuthRepository:
    ...
    async def save_refresh_token(self,data:SaveRefreshToken):
        ...
        async with self.conn.cursor(DictCursor) as cur:
            await cur.execute(sql,(data.user_id,data.token,data.expires_at,data.is_revoked))
    ...
    async def old_token_change_new_token(self,old_token:str,new_token:str,expires_at:datetime)-> bool:
        sql="""
             UPDATE
             refresh_tokens
             SET
